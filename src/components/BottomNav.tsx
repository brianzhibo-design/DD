...
export default function BottomNav({ onMoreClick }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
      {/* 背景 */}
      <div className="absolute inset-0 bg-white border-t border-[#E2E8D5]" />
      ...
      <div className="relative flex items-center justify-around px-2 pb-safe">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center py-2 px-4 min-h-[60px] min-w-[60px] rounded-lg transition-all ${
                isActive 
                  ? 'text-[#4A6741]' 
                  : 'text-[#7D8A80] active:text-[#4A6741] active:bg-[#4A6741]/5'
              }`}
            >
              <div className={`relative ${isActive ? 'scale-110' : ''} transition-transform`}>
                <item.icon size={22} strokeWidth={isActive ? 2.5 : 2} />
                {isActive && (
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-[#4A6741] rounded-full" />
                )}
              </div>
        ...
        <button
          onClick={onMoreClick}
          className="flex flex-col items-center justify-center py-2 px-4 min-h-[60px] min-w-[60px] rounded-lg text-[#7D8A80] active:text-[#4A6741] active:bg-[#4A6741]/5 transition-all"
          aria-label="更多选项"
        >
